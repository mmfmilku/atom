...
public class JTerminal {
    ...
    public static JTerminalResult executeTerminal(String id, String code) {
        ...
        synchronized (JTerminalExecutor.class) {
            ...
            try {
                // 保存执行结果
                Object executeReturn = executor.execute(jTerminalDomain.getContextVars());
    ...
    private static JavaAST getJavaAST(String code, JTerminalDomain jTerminalDomain) {
        ...
        // 使用全局import
        javaAST.setImports(jTerminalDomain.getImportList());
    ...
    private static Statement enhanceStatement(Statement statement, JTerminalDomain jTerminalDomain) {
            ...
            for (Expression baseExp : expression.getLeafExpression()) {
                if (baseExp instanceof Identifier) {
                    ...
                    if (jTerminalDomain.getContextVars().containsKey(value)
                            || jTerminalDomain.getCurrVars().contains(value)) {
        ...
        if (statement instanceof NestedStatement) {
            ...
            for (Statement codeBlockStatement : codeBlock.getStatements()) {
                enhanceStatement(codeBlockStatement, jTerminalDomain);
            }
            // TODO
